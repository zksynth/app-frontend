	...
	return (
		<>
			<Head>
				...
				<Box w={'100%'}>
					<Box pt={20} bg={'white'} pb={4} mt={-7} border={'1px'} borderColor={'#e8e8e8'} px={5} rounded={'18px'}>
						<Market/>
					</Box>
					<Box py={2}>
						<Position/>
						...
						{((pools[tradingPool]?.paused || !isMarketOpen(pools[tradingPool]?.name ?? 'Crypto Market'))) ?(
							...
							>
								...
								>
									...
									>
										<Box className={`${VARIANT}-${colorMode}-containerBody`} shadow={'0'} h={'100%'} mr={2}>
											<CollateralTable />
										</Box>
									</motion.div>
								</Box>
								<Box w={{ sm: "100%", md: "67%" }}>
									...
									>
										<Box className={`${VARIANT}-${colorMode}-containerBody`} shadow={'0'} h={'100%'} ml={2}>
											<IssuanceTable />
										</Box>
									</motion.div>
								</Box>
